...
function latest_notice($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='', $category='')
{
    global $g5;
    //static $css = array();
    ...
    $cache_fwrite = true;
    if(G5_USE_CACHE) {
        $cache_file = G5_DATA_PATH."/cache/latest-{$bo_table}-{$skin_dir}-{$rows}-{$subject_len}-notice.php";
    ...
    if(!G5_USE_CACHE || $cache_fwrite) {
		...
		$board_notice_id = sql_fetch($sql);
		$notice_id=$board_notice_id['bo_notice'];
			
        $tmp_write_table = $g5['write_prefix'] . $bo_table; // 게시판 테이블 전체이름
        $sql = " select * from {$tmp_write_table} where wr_id IN ( $notice_id ) and ca_name='{$category}' order by wr_num limit 0, {$rows} ";
