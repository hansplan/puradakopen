...
$action_url = "/adm/bbs/write_update.php";
...
<section id="bo_w">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td><input type="text" name="wr_name" value="<?php echo $wr_1 ?>" id="wr_name" required class="frm_input required" size="10" maxlength="20"></td>
        </tr>
        <?php } ?>
		...
		<style>
			#wr_content{height:inherit !important}
			.tbl_frm01 span{color:white;padding:3px 8px;border-radius:3px;margin:0 5px;}
		...
		<?
				...
				function Num_String($val){
					if( is_numeric($val)){
						echo  '￦ '.number_format($val).' 원';
					}else{
						echo	$val;
					}
				}
		...
		<tr>
			<th>상담 진행 상태</th>
			<td>
			<select class="Address_D col-xs-6 custom-select" id="result" title="접수" name="wr_7" >
				<option value="1" <?php echo get_selected($wr_7, '1') ; ?>>접수</option>
				<option value="2"<?php echo get_selected($wr_7, '2') ; ?>>진행중/보류</option>
				<option value="3"<?php echo get_selected($wr_7, '3') ; ?>>상담완료</option>
			</select>
			</td>
		</tr>

        <tr>
            <th scope="row"><label for="wr_content">응대 내용<strong class="sound_only">필수</strong></label></th>
            <td class="wr_content">
		...
		<tr style="background:#edeeef;">
			...
			<td>
					이름 : <?= $wr_1?><br>
					연락처 : <?= $wr_2?><br>
					예산 : <?= Num_String($wr_3)?><br>
					희망지역 : <?= $wr_4?><br>
					창업계기 : <?= pathway($wr_5)?><br>
			</td>
		</tr>
